...
class WeDevs_Settings_API {
    ...
    function callback_select( $args ) {
        ...
        foreach ( $args['options'] as $key => $label ) {
            $html .= sprintf( '<option value="%s"%s>%s</option>', $key, apply_filters('wc_ao_select_'.$args['section'],selected( $value, $key, false )), $label );
           /* apply_filters('wc_ao_select_'.$args['section'],selected( $value, $key, false ));*/
        }
        $html .= sprintf( '</select>' );
        $html .= sprintf( '<span class="description"> %s</span>', $args['desc'] );
    ...
    function get_option( $option, $section, $default = '' ) {
        ...
        $options = apply_filters('wc_ao_get_options',$options);

        if ( isset( $options[$option] ) ) {
            return apply_filters('wc_ao_get_option',$options[$option]);
        }
    ...
    function script() {
        ...
        <style type="text/css">
            /** WordPress 3.8 Fix **/
            form h2,.form-table th { padding: 20px 10px; }
